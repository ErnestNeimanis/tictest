...
diagonalCombinationsRight(fieldSize, comboLength){
 ...
 createAllCombinations(fieldSize, comboLength){
...
allPossibleEntriesInCombo(pfv,combo){
    return combo.map(element => pfv[element.row][element.col] === 0)
}

choseRandomCombo(pfv,allCombos, turnId = 2){
...
arrayOccurances (array, element){
    return array.reduce((count, val) => (val == element ? count +1 : count), 0);
}

allEmergingCombos = (pfv,allCombos,turnId,threshold) => {
...
blockNecessary(pfv,allCombos,opponentId,threshold){
    return allEmergingCombos(pfv,allCombos,opponentId,threshold) > 0;
}

randomBlock(pfv,allCombos,opponentId,threshold, callback){
...
mostFrequentMultipleRandom (array){
...
prioratizedBlocking (pfv,allCombos,comboLength,opponentId,threshold,callback) {

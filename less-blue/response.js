...
export class Response extends ComboUtils {
  ...
  // _createResponse() {
  //   if (
  //     typeof this.responseData.row === "number" &&
  //     typeof this.responseData.col === "number"
  //   ) {
  //     this._fillInCellData(
  //       new Cell(this.responseData.row, this.responseData.col)
  //     );
  //   }
  //   if (
  //     this.responseData.combo &&
  //     typeof this.responseData.id !== "undefined"
  //   ) {
  //     this._fillInWinnerData(
  //       new Winner(this.responseData.combo, this.responseData.id)
  //     );
  //   } else {
  ...
  _createResponse() {
    ...
    responseData.forEach((obj) => {
      if (
        ...
        this._fillInCellData( new Cell(obj.row, obj.col));
        success = true;
      }
      if (obj.combo && typeof obj.id !== "undefined"
      ) {
        this._fillInWinnerData(new Winner(obj.combo, obj.id));
        success = true;
      }
    });
  ...
  _checkArguments(gameData, responseData) {
    ...
    if (!responseData) {
      throw new Error(
        "'Response' must have argument 'responseData' of type 'Cell' or 'Winner'!"
      );
    }
  }
}
